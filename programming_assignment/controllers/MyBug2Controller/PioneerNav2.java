...
public class PioneerNav2 {
  ...
  public PioneerProxSensors1 prox_sensors;
  ...
  public void go_to_target(double robot_linearvelocity, double scaling_factor, boolean right, double deltaTheta) {
    int direction_coeff = 1;
    if (right) direction_coeff = -1;
    this.set_velocity(robot_linearvelocity, scaling_factor * deltaTheta * direction_coeff);
    this.state = MoveState.GO_TO_TARGET;
  }
  ...
  public boolean is_encountered_obstacle(double set_point) {
    if (Math.min(this.prox_sensors.get_value(1),
          Math.min(this.prox_sensors.get_value(2),
            Math.min(this.prox_sensors.get_value(3),
              Math.min(this.prox_sensors.get_value(4),
